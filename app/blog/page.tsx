				...
				<div className="absolute inset-0 flex items-center justify-center">
					<div className="flex h-full w-full max-w-[1500px] items-center justify-between p-4">
						...
						<div className="hidden w-1/3 bg-secondary p-4 text-lg lg:flex">
							<h3>
								Welcome to the RottenBrains Developer Blog
								(Beta). Where you can find all the news about
								the website, my perfect coding skills and me
								slowly losing my sanity with databases.
							</h3>
						</div>
					</div>
				</div>
				...
				<div className="p-4 lg:w-2/3">
					...
					<div className="flex w-full flex-col gap-4 lg:grid lg:grid-cols-[repeat(auto-fit,minmax(300px,1fr))]">
						{posts &&
							posts.length > 0 &&
							posts.map((post) => (
								<PostCardMain key={post.id} post={post} />
							))}
					</div>
				</div>

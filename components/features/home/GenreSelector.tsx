...
const GenreSelector: React.FC<GenreSelectorProps> = ({
	...
	return (
		<div className="hidden-scrollbar top-20 -mb-4 flex h-12 flex-row items-center gap-2 overflow-x-auto bg-background px-2 py-2 text-sm transition-all duration-300 lg:sticky lg:z-30 lg:h-14 lg:px-0 lg:pb-4">
			{/* Category buttons */}
			{user ? (
				<button
			...
			>
				<div className="w-full">
					...
					<p className="mb-4 text-sm text-foreground/60">
						These genres will be used to build your custom feed. You
						can also filter out the content on your feed by removing
						the genres you don&apos;t want to see on your home page.
					</p>
					...
					<div className="mb-4 flex w-full flex-row flex-wrap gap-4">
						{movieGenres
							...
							.map((genre) => (
								<button
									onClick={() =>
										removeGenreFromFeed(genre.id, "movie")
									}
									key={genre.id}
					...
					<div className="mb-8 flex w-full flex-row flex-wrap gap-4">
						{tvGenres
							...
							.map((genre) => (
								<button
									onClick={() =>
										removeGenreFromFeed(genre.id, "tv")
									}
									key={genre.id}
									className="flex flex-shrink-0 flex-row items-center gap-2 rounded-[8px] bg-foreground px-3 py-1 text-background"
								>
					...
					<p className="mb-4 text-sm text-foreground/60">
						Genres that are here will not be included in your
						recommendations and will not show up on your home page.
					</p>
